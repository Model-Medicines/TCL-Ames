...
def dim_reduce(df, test_df, model_path, col_name1, weight, scaler_output_path=None):
    ...
    if scaler_output_path is not None:
        artifact_utils.dump_joblib(sc, scaler_output_path)


    ### create and train model (instead of loading pre-trained weights)
    n_dim = X.shape[1]  # Number of input features (dynamic based on selected base classifiers)
    model = create_model(n_dim, node=32, activation='relu', optimizer=optimizers.SGD(lr=0.1))
    best_model = fit_model(X, y, X_val, y_val, n=100, model_path=model_path, model=model, batch_size=32, weight=weight)
...
def deepames_prediction(probability_path, name, weight, result_path, model_output_dir=None, schema_manifest_path=None, model_manifest_dir=None):
    ###data
    ...
    # Use provided model_output_dir or default to ./DeepAmes_models
    if model_output_dir is not None:
        model_path = model_output_dir + '/weight_'+str(weight)+'.h5'
    else:
        model_path = './DeepAmes_models/weight_'+str(weight)+'.h5'
    if model_manifest_dir is None:
        if model_output_dir is not None:
            model_manifest_dir = model_output_dir
        else:
            model_manifest_dir = './DeepAmes_models'
    scaler_path = model_manifest_dir + '/weight_'+str(weight)+'_scaler.joblib'
    manifest_path = model_manifest_dir + '/weight_'+str(weight)+'_manifest.json'

    ###get the prediction
    validation_class, validation_result, test_class, test_result = dim_reduce(
        data, test, model_path, col_name, weight, scaler_output_path=scaler_path
    )
    ...
    manifest_data = {
        ...
        'standard_scaler_path': os.path.abspath(scaler_path),
        'schema_manifest_path': os.path.abspath(schema_manifest_path) if schema_manifest_path is not None else None,
        'validation_probabilities_file': os.path.abspath(probability_path+'/validation_probabilities_' + name + '.csv'),
        'test_probabilities_file': os.path.abspath(probability_path+'/test_probabilities_' + name + '.csv')
    }
    artifact_utils.write_json(manifest_data, manifest_path)
