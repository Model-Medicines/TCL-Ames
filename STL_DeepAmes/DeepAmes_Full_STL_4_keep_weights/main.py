...
def run_pipeline(train_data_path, test_data_path, features_path, output_dir, name):
    ...
    artifact_path = workDir + '/artifacts'
    base_artifact_path = artifact_path + '/base'
    schema_manifest_path = artifact_path + '/schema/validation_schema_' + name + '.json'
    deepames_artifact_path = artifact_path + '/deepames'
    selected_base_artifact_path = artifact_path + '/selected_base_artifacts.csv'
    run_manifest_path = artifact_path + '/run_manifest_' + name + '.json'
    ...
    os.makedirs(os.path.dirname(schema_manifest_path), exist_ok=True)
    os.makedirs(deepames_artifact_path, exist_ok=True)

    ### run the scripts
    base_knn.generate_baseClassifiers(features, data, external, base_path+'/knn', artifact_dir=base_artifact_path+'/knn')
    base_lr.generate_baseClassifiers(features, data, external, base_path+'/lr', artifact_dir=base_artifact_path+'/lr')
    base_svm.generate_baseClassifiers(features, data, external, base_path+'/svm', artifact_dir=base_artifact_path+'/svm')
    base_rf.generate_baseClassifiers(features, data, external, base_path+'/rf', artifact_dir=base_artifact_path+'/rf')
    base_xgboost.generate_baseClassifiers(features, data, external, base_path+'/xgboost', artifact_dir=base_artifact_path+'/xgboost')

    mcc = select_base.select_base_classifiers(
        base_path,
        artifact_base_path=base_artifact_path,
        selected_artifact_output_path=selected_base_artifact_path
    )

    validation_predictions_combine.combine_validation_probabilities(
        base_path, mcc, probability_path, name, schema_manifest_path=schema_manifest_path
    )
    test_predictions_combine.combine_test_probabilities(base_path, mcc, probability_path, name)

    deepames_weight_manifests = []
    for weight in tqdm(range(6, 19, 1), desc="DeepAmes+ Weights"):
        weight_manifest_path = deepames_plus.deepames_prediction(
            probability_path,
            name,
            weight,
            result_path,
            ...
            model_manifest_dir=deepames_artifact_path
        )
        deepames_weight_manifests.append(weight_manifest_path)
    ...
    run_manifest = {
        ...
        'selected_base_manifest': os.path.abspath(selected_base_artifact_path),
        'deepames_schema_manifest': os.path.abspath(schema_manifest_path),
        'deepames_weight_manifests': deepames_weight_manifests
    }
    artifact_utils.write_json(run_manifest, run_manifest_path)
