...
base_dir = project_root / "All_Results"
# Directory containing the test data with SMILES
test_data_dir = project_root / "Ready_Data" / "Test_Data_Featurized"
...
def get_smiles_for_task(task_name):
    ...
    if task_name not in smiles_cache:
        ...
        if test_file.exists():
            test_df = pd.read_csv(test_file, usecols=["SMILES"]).reset_index()
            test_df = test_df.rename(columns={"index": "id"})
            smiles_cache[task_name] = test_df[["id", "SMILES"]]
        else:
...
for dir_name, csv_list in results.items():
    ...
    smiles_df = get_smiles_for_task(curr_task_name)
    for curr_csv in csv_list:
        ...
        if smiles_df is not None:
            curr_sub_df = curr_sub_df.merge(smiles_df, on="id", how="left", validate="one_to_one")
        else:
