...
def combine_validation_probabilities(base_path, mcc, probability_path, name, schema_manifest_path=None):
    ...
    output_csv_path = join(probability_path+'/validation_probabilities_' + name + '.csv')
    data.to_csv(output_csv_path)

    if schema_manifest_path is None:
        schema_manifest_path = join(probability_path, 'validation_schema_' + name + '.json')
    feature_columns = [col for col in data.columns if col not in ['id', 'y_true']]
    schema_manifest = {
        'dataset_name': name,
        'source_validation_probabilities_file': os.path.abspath(output_csv_path),
        'feature_columns': feature_columns,
        'feature_count': len(feature_columns),
        'feature_columns_sha256': artifact_utils.sha256_for_columns(feature_columns)
    }
    artifact_utils.write_json(schema_manifest, schema_manifest_path)
