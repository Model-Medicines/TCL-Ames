'''
Standalone copy of gmtames results.py — no gmtames package imports.
Run from any directory: python results_standalone.py [path_to_output]
path_to_output must contain a test_predictions/ folder; results go to test_results/.
'''
...
# Inlined so this script has no gmtames dependency
...
def mcc_score(y_true, y_pred):
    return matthews_corrcoef(y_true, np.rint(y_pred))
...
def calculateResults(path_to_output):
    ...
    for f in test_predictions_dir:
        ...
        for task in task_list:
            ...
            task_intervals_90 = calculateConfidenceIntervals(task_estimated, level=0.90)  # DataFrame[metrics, lower/upper bounds]

            task_results = pd.concat([task_observed, task_intervals_90], axis=1)  # DataFrame[metrics, statistics]
    ...
    full_results.sort_values([('Balanced accuracy', 'Observed'), ('ROC AUC', 'Observed'), ('MCC', 'Observed'), ('Sensitivity', 'Observed'), ('Specificity', 'Observed')], ascending=False, kind='stable', inplace=True)
    ...
    def computeSignificance(df):
        ...
        for task in df.index.unique(level='Strain task'):
            st_upper_90 = df.loc[(task, 'STL'), (metric, '95.0%')].values[0]
            for architecture, grouping in df.loc[task].index:
                significance = 'NS'
                if architecture == 'STL': significance = '-'
                else:
                    mt_lower_90 = df.loc[(task, architecture, grouping), (metric, '5.0%')]
                    if mt_lower_90 > st_upper_90: significance = '*'
    ...
    def _parseStatistics(df):
        ...
        for indices in df.index:
            parsed = '%.3f (%.3f-%.3f)' % (
                ...
                df.loc[indices, (metric_full, '5.0%')],
                df.loc[indices, (metric_full, '95.0%')]
            )
    ...
    task_results = task_results.loc[:, ['Balanced accuracy', 'MCC', 'Sensitivity', 'Specificity', 'ROC AUC']]
    ...
    overall_results.sort_values([('Balanced accuracy full', 'Observed'), ('ROC AUC full', 'Observed'), ('MCC full', 'Observed'), ('Sensitivity full', 'Observed'), ('Specificity full', 'Observed')], ascending=False, kind='stable', inplace=True)
    ...
    def _computeSignificanceMod(df):
        metric = df.columns.unique(level='Metric')[0]
        st_upper_90 = df.loc['STL', (metric, '95.0%')]
        for architecture in df.index:
            significance = 'NS'
            if architecture == 'STL': significance = '-'
            else:
                mt_lower_90 = df.loc[architecture, (metric, '5.0%')]
                if mt_lower_90 > st_upper_90: significance = '*'
    ...
    averaged_results = averaged_results.loc[:, ['Balanced accuracy', 'MCC', 'Sensitivity', 'Specificity', 'ROC AUC']]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Compute gmtames test results from test_predictions (standalone, no gmtames package).')
    parser.add_argument(
        'path_to_output',
        type=pathlib.Path,
        nargs='?',
        default=pathlib.Path(__file__).resolve().parent.parent / 'output' / 'mtg_experiment_scaffold',
        help='Directory containing test_predictions/ (default: .../output/mtg_experiment_scaffold)',
    )
    args = parser.parse_args()
    calculateResults(args.path_to_output)
    print('Results written to', args.path_to_output / 'test_results')
