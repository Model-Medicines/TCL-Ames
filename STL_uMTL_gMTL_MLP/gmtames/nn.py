...
def checkDevice(device):
    ...
    elif device.lower() == 'mps' and torch.backends.mps.is_available(): return 'mps'
    elif device.lower().startswith('cuda') and torch.cuda.is_available(): return device.lower()
...
def trainNTaskNeuralNetwork(model, tasks, train_dataloader, n_epochs, lr):
    ...
    # Precompute weight masks once (labels don't change during training)
    # Use weighted loss to avoid dynamic tensor indexing (no CPU-GPU sync)
    precomputed_weights = None
    precomputed_y_train = None
    precomputed_X_train = None
    n_valid_per_task = []
    
    for X_train, y_train in train_dataloader:
        precomputed_X_train = X_train
        # Replace FILTER_VALUE with 0 in y_train (won't affect loss due to zero weight)
        precomputed_y_train = torch.where(y_train == FILTER_VALUE, torch.zeros_like(y_train), y_train)
        # Create weight tensor: 1 for valid, 0 for filtered
        precomputed_weights = (y_train != FILTER_VALUE).float()
        # Count valid samples per task for proper averaging
        for task_idx in range(len(tasks)):
            n_valid_per_task.append(precomputed_weights[:, task_idx].sum())
        break  # Only need to compute once

    model.train()
    for epoch in range(n_epochs):
        # Forward pass (use precomputed X to avoid re-fetching)
        y_pred = model.forward(precomputed_X_train)
       
        # Compute weighted BCE loss for all tasks at once
        # BCE = -[y*log(p) + (1-y)*log(1-p)]
        eps = 1e-7
        bce_per_sample = -(precomputed_y_train * torch.log(y_pred + eps) + 
                          (1 - precomputed_y_train) * torch.log(1 - y_pred + eps))
        
        # Apply weights and sum per task, then average by valid count
        weighted_bce = bce_per_sample * precomputed_weights
        loss_list = []
        for task_idx in range(len(tasks)):
            task_loss = weighted_bce[:, task_idx].sum() / n_valid_per_task[task_idx]
