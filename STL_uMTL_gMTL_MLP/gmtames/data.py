...
# Master data lives in master_datasets
PATH_TO_MASTER_DATASETS = _PROJECT_ROOT / 'gmtames' / 'data' / 'master_datasets'
PATH_TO_BASE_DATASETS = _PROJECT_ROOT / 'gmtames' / 'data' / 'base_datasets'
PATH_TO_AD_DATASETS = _PROJECT_ROOT / 'gmtames' / 'data' / 'ad_datasets'
...
def generateBaseDatasets(testsplit):
    ...
    path_to_base_datasets_testsplit.mkdir(parents=True, exist_ok=True)
...
def generateModellingDatasets(selected_tasks, testsplit):
    ...
    for t, dataset in modelling_datasets.items():
        ...
        # Ensure all feature columns (fingerprints) are numeric
        # The fingerprint columns are from index 1 to last_n_cols (excluding ID and endpoints)
        feature_cols = dataset.columns[1:last_n_cols]
        dataset[feature_cols] = dataset[feature_cols].apply(pd.to_numeric, errors='coerce').fillna(0)
        
        # Ensure endpoint columns are numeric
        endpoint_cols = dataset.columns[last_n_cols:]
        dataset[endpoint_cols] = dataset[endpoint_cols].apply(pd.to_numeric, errors='coerce')
...
def generateApplicabilityDomainDatasets(testsplit):
    ...
    path_to_ad_datasets_testsplit.mkdir(parents=True, exist_ok=True)
